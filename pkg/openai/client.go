...
const wordExamplesMessage = `Give me two very simple Chinese example sentences for the usage of theChinese word provided by the user. Use simplified Chinese characters. Also add the pinyin and the english translation. Serialize the response into a JSON dict and add the sentences in a JSON array that is referenced by the key "examples". Each example sentence in the array should be a JSON dict which has the following fields:
